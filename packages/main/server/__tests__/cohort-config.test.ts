 ...
 * packages/shared/cohort-config.ts) that exports the boundaries and labels,
...
describe('cohort configurability', () => {
  it('a shared cohort config file exists', () => {
    ...
    let exists = false;
    try {
      readSrc('packages/shared/cohort-config.ts');
      exists = true;
    } catch {
      // file doesn't exist yet
    }
    expect(exists, 'packages/shared/cohort-config.ts should exist as the single source of truth for cohort boundaries and labels').toBe(true);
  });

  it('analytics-cohorts.ts imports cohort boundaries from shared config', () => {
    const source = readSrc('packages/main/server/services/analytics-cohorts.ts');
    expect(
      source,
  ...
  it('narratives.ts imports cohort labels from shared config', () => {
    const source = readSrc('packages/main/client/lib/narratives.ts');
    expect(
      source,
  ...
  it('CohortAreaChart.tsx imports cohort config from shared config', () => {
    const source = readSrc('packages/shared/components/charts/CohortAreaChart.tsx');
    expect(
      source,
  ...
  it('ContributorTable.tsx imports cohort color map from shared config', () => {
    const source = readSrc('packages/main/client/components/ContributorTable.tsx');
    expect(
      source,
  ...
  it('chartTransforms.ts uses dynamic cohort keys instead of hardcoded new/mid/senior', () => {
    const source = readSrc('packages/shared/lib/chartTransforms.ts');
    expect(
      source,
  ...
  it('CohortLabel type is derived from config, not a hardcoded union', () => {
    const source = readSrc('packages/shared/types.ts');
